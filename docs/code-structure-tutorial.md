...
flowchart TD
    ...
    K -->|no| M["drop event"]
    N["setIdentity(identity)"] --> O["future events use identity"]
    O --> J
...
client.identify(userId, sessionId, context)
...
초기 `init()` 시 `page_view`를 capture합니다. SPA route 변경은
`history.pushState`, `history.replaceState`, `popstate`, `hashchange`로 감지합니다.
URL이 실제로 바뀐 경우에만 새 `page_view`를 만듭니다.
