...
현재 이 저장소는 SDK source와 build 설정을 관리합니다. npm registry 배포나 CDN
업로드 자동화는 아직 포함하지 않습니다.

로컬 또는 데모에서 browser bundle을 만들 때는 아래 명령을 사용합니다.
...
패키지로 배포된 뒤에는 앱 부팅 시 SDK를 먼저 시작합니다. 이 시점에는 로그인
사용자 정보가 없어도 됩니다.
...
`npm run build`로 만든 IIFE bundle을 CDN, S3/CloudFront, 정적 파일 서버에 올린 뒤
그 URL을 사용합니다. 로컬 예시는 [examples/basic.html](examples/basic.html)을
참고합니다.

```html
<script src="/assets/loop-ad-event-sdk.iife.js"></script>
<script>
